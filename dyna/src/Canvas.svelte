<script> 
    ...
    function spawn(config) { 
        ...
        if (config.kind === 'conversations') {
            const win = {
            id, kind: 'conversations',
            title: config.title ?? 'Conversations',
            position: pos, size, z: ++zCounter, persist
            };
            windows = [...windows, win];
            return id;
        }
        ...
        // Conversation thread
        if (config.kind === 'conversationThread') {
            const win = {
                id, kind: 'conversationThread',
                title: config.title ?? (config.conversation?.title ?? 'Conversation'),
                conversation: {
                id: String(config.conversation?.id || ''),
                title: String(config.conversation?.title || '')
            },
            position: pos, size, z: ++zCounter, persist
            };
            windows = [...windows, win];
        ...
        if (config.kind === 'agentSessions') {
            const win = {
            id, kind: 'agentSessions',
            title: config.title ?? 'Agent Sessions',
...
const cid = e?.detail?.conversation_id;
const ctitle = e?.detail?.title ?? 'Conversation';
if (!cid) return;

// Center and right panels; sizes can be adjusted as you like
spawn({
...
position: {x:460, y:40},
size: { w: 1200, h: 800 },
conversation: { id: cid, title: ctitle }
});
